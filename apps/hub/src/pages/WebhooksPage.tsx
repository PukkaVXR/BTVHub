...
import { Button, CopyField, EmptyState, FormField, PageHeader, SplitWorkspace, StatusPill } from "../ui";
...
export default function WebhooksPage() {
  ...
  return (
    <>
      ...
      <SplitWorkspace
        className="webhook-workspace"
        sidebarClassName="webhook-list"
        detailClassName="webhook-detail"
        sidebarLabel="Webhook endpoints"
        sidebar={
          <>
            {hooks.map((hook) => (
              <button
              ...
              >
                ...
                <StatusPill
                  tone={hook.secret ? "success" : "neutral"}
                  label={hook.secret ? "Secret configured" : "No secret"}
                />
              </button>
            ))}
            {!hooks.length && (
          ...
          editing ? (
            <div className="card webhook-editor-card">
              ...
              <div className="ui-callout ui-callout--warning">
                Webhooks now require authentication. Send either <code>X-BTV-Secret</code> with this secret, or{" "}
                <code>X-BTV-Signature</code> as an HMAC SHA-256 signature of the JSON payload.
              </div>
              <FormField
                label="Secret header (X-BTV-Secret)"
                hint={
                  editing.secret
                    ? "Configured. Generate a new value to rotate the secret, then update the external sender."
                    : "Required before this webhook can be saved or triggered."
                }
              >
                <input
              ...
              <div className="actions" style={{ marginTop: 8, marginBottom: 12 }}>
                <Button
                  ...
                  variant="secondary"
                  size="sm"
                  onClick={() => setEditing({ ...editing, secret: generateWebhookSecret() })}
                >
                  Generate / rotate secret
                </Button>
                {editing.secret ? (
                  <StatusPill tone="success" label="Secret configured" />
                ) : (
                  <StatusPill tone="danger" label="Secret required" />
                )}
              </div>
              <div className="form-row">
                <label>Action</label>
              ...
              <div className="actions" style={{ marginBottom: 12 }}>
                <button
                  ...
                  onClick={() => setActionConfigJson(JSON.stringify(configTemplate("alert"), null, 2))}
                >
                  Alert config
                </button>
                <button
                  type="button"
                  className="ui-button ui-button--secondary ui-button--sm"
                  onClick={() => setActionConfigJson(JSON.stringify(configTemplate("goal_increment"), null, 2))}
                >
                  Goal config
                </button>
                <button
                  type="button"
                  className="ui-button ui-button--secondary ui-button--sm"
                  onClick={() => setActionConfigJson(JSON.stringify(configTemplate("effect"), null, 2))}
                >
                  Effect config
                </button>
                <button
                  type="button"
                  className="ui-button ui-button--secondary ui-button--sm"
                  onClick={() => setActionConfigJson(JSON.stringify(configTemplate("macro"), null, 2))}
                >
                  Macro config
                </button>
              </div>
            ...
            <EmptyState
              ...
              description="Choose an endpoint from the list or create a new one to configure its action and secret."
            />
          )
        }
      />
      ...
      <div className="card webhook-log-card">
        ...
        <div className="webhook-log-list">
          ...
          {!log.length && (
            <EmptyState
              title="No webhook requests yet"
              description="Requests will appear here after an external sender posts to a webhook URL."
            />
          )}
        </div>
      </div>
